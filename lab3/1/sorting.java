...
String input = "/lab3_1/input-5m";
String temp = "/lab3_1/temp";
String output = "/lab3_1/output";
...
// For the first MR round, we just need to convert data into sequencefile format
// As a result, we don't need any reduce task. However, In this lab wedon't
// need to do this, because we are looking for ASCII sort and theSequenceFileFormat
...
// This is not allowed
// FileOutputFormat.setOutputPath(job_one, newPath(another_output_path));
// Run the job
job_one.waitForCompletion(true);
// Create job for round 2
// The output of the previous job can be passed as the input to thenext
...
// There are also parameters for the maximum number of samples to takeand the maximum
// number of splits to sample
job_two.setPartitionerClass(TotalOrderPartitioner.class);
InputSampler.Sampler<IntWritable, Text> sampler = new InputSampler.RandomSampler<IntWritable, Text>(0.01, 1000, 10);
...
// In the second reduce function, we already have sorted values in eachreducer and because of having
// the sampler partitioner we have balanced output files which files are alsosorted such that R1 contains
// smallest and all data in R2 are smaller than R3 and so on. Therefore, Ijust emit the obtained (key,
